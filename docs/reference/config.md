...
| `dolt_memory_limit` | string |  |  | DoltMemoryLimit bounds the managed dolt sql-server's Go heap by setting GOMEMLIMIT on the dolt process. Free-form string passed verbatim to GOMEMLIMIT (e.g. "6000MiB", "4GiB", or a raw byte count). Empty (the default) injects nothing, preserving today's unbounded behavior. Opt in to stop dolt's anonymous-memory growth from exhausting host RAM — especially under WSL2, where the vmmem RAM cap plus swap.vhdx thrash freeze the host with no OOM-kill. An explicit GOMEMLIMIT already in the dolt process environment wins over this value. |
